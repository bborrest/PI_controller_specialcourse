...
%% Mooring data
global rho_H2O
ld = 39.9;                                      % delta lines projected length (43.9)
hd = 2.93;                                      % delta lines projected height of connection adjusted (2.48)
...
psi = X(6);
% rotation matrix
R = [cos(psi)*cos(theta), cos(psi)*sin(theta)*sin(phi), cos(psi)*sin(theta)*cos(phi) + sin(psi)*sin(phi);
...
dR_dphi = [0, cos(psi)*sin(theta)*cos(phi) + sin(psi), - cos(psi)*sin(theta)*sin(phi) + sin(psi)*cos(phi);
    0, sin(psi)*sin(theta)*cos(phi), -sin(psi)*sin(theta)*sin(phi) - cos(psi)*cos(phi);
    0, cos(theta)*cos(phi), -cos(theta)*sin(phi)];
dR_dtheta = [-cos(psi)*sin(theta), cos(psi)*cos(theta)*sin(phi), cos(psi)*cos(theta)*cos(phi);
    -sin(psi)*sin(theta), sin(psi)*cos(theta)*sin(phi), sin(psi)*cos(theta)*cos(phi);
    -cos(theta), -sin(theta)*sin(phi), -sin(theta)*cos(phi)];
dR_dpsi = [-sin(psi)*cos(theta), -sin(psi)*sin(theta)*sin(phi) - cos(psi)*cos(phi), -sin(psi)*sin(theta)*cos(phi) + cos(psi)*sin(phi);
    cos(psi)*cos(phi), cos(psi)*sin(theta)*sin(phi) - sin(psi)*cos(phi), cos(psi)*sin(theta)*cos(phi) + sin(psi)*sin(phi);
    0, 0, 0];
% rm (fairlead attachment location)- position partial derivatives
drm_dphi = dR_dphi*r_po;
% these will have to be fixed for 3x3 rpo
dXp_dphi = drm_dphi(1);
dYp_dphi = drm_dphi(2);
dZp_dphi = drm_dphi(3);
